...
void funzione_comando_cd(char buffer[]){
    ...
    dir_corrente=getcwd(buf, sizeof(buf));
    
    dir=strcat(dir_corrente, path);
...
void executer(int socket_id, char buffer[], char *canale, char *utente){
        ...
        sprintf(COMANDO,"%s > %s",comando, output);
        ...
        sprintf(msg, "PRIVMSG %s :%s", canale, messaggio);
